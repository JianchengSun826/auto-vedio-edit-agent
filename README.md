...
工具在本地运行 WhisperX 对素材完成语音转录，生成带时间码的逐句字幕。剪辑师输入主题词或采访问题，系统在字幕中定位所有相关声效（soundbite），自动生成带 in/out 点的粗剪列表，供人工审核确认后直接导出。
...
每次点击「开始分析」调用一次 API，费用取决于视频时长（决定转录文本长度）：

| 视频时长 | 约消耗 token | 单次费用估算 |
|---------|------------|------------|
| 5 分钟 | ~1,500 tokens | 约 ¥0.01 |
| 30 分钟 | ~5,000 tokens | 约 ¥0.03 |
...
- 剪辑师每天处理 3 条采访（各 30 分钟）：约 ¥0.09/天
- 每月处理 100 个视频（平均各 20 分钟）：约 ¥2/月
- 一次性处理 50 小时存档素材：约 ¥3.5

### 控制费用的建议

- **同一视频多次修改指令**：只要不刷新页面，重新点击「开始分析」才会再次计费；直接在候选列表里勾选/取消片段不计费
- **降低 Whisper 模型精度**：在 `docker.env` 中将 `WHISPER_MODEL=medium` 改为 `small` 可减少转录时间，但不影响 API 费用
- **查看账单**：登录 [console.anthropic.com](https://console.anthropic.com) → Usage 页面可查看实时用量和费用
...
| 步骤 | 说明 | 参考耗时 |
|------|------|---------|
| 转录 | WhisperX 将视频音频转为带时间戳的文字，可选说话人分离 | 10 分钟视频约 1–3 分钟 |
...
|------|--------|---------|---------|------|------|
| 1 | SPEAKER_00 | 12.3s – 28.5s | 竞品的价格策略非常激进，我们... | 1.00 | ☑ |
| 2 | SPEAKER_01 | 45.0s – 62.1s | 跟竞品相比，我们在续航方面... | 1.00 | ☑ |
| 3 | SPEAKER_00 | 120.5s – 135.0s | 说到价格，用户反馈最多的是... | 1.00 | ☑ |

- **取消勾选**不想要的片段
- **说话人列**：配置 HuggingFace Token 后显示说话人标签（SPEAKER_00 等），未配置时显示"—"
...
**按说话人提取（需配置 HuggingFace Token）：**
```
提取 SPEAKER_00 所有关于产品价格的回答
```
```
只保留记者的提问部分（SPEAKER_01），去掉被采访者的回答
```
```
把 SPEAKER_00 的所有发言拼成一条完整视频，导出到 B 站
```
...
├── processing/
│   ├── transcriber.py       ← 语音转录（WhisperX + 说话人分离）
...
- **多语言**：WhisperX 支持 99 种语言，修改转录参数即可启用
