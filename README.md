...
- [个人剪辑偏好配置](#个人剪辑偏好配置)
- [界面使用流程](#界面使用流程)
  - [方式 A：快捷功能按钮（免费）](#方式-a--快捷功能按钮免费无需-ai)
  - [方式 B：文字需求（AI 解析）](#方式-b--自定义文字需求ai-解析少量费用)
  - [按说话人剪辑两阶段流程](#按说话人剪辑的两阶段流程)
- [功能详解](#功能详解)
  - [三种剪辑模式](#三种剪辑模式)
  - [四个导出平台](#四个导出平台)
  - [指令写法参考](#指令写法参考)
...
| 操作路径 | 触发方式 | 费用 |
|---------|---------|------|
| **快捷功能按钮**（关键词、说话人、时间段、去静音） | 点击按钮后直接执行，不调用 AI | **完全免费** |
| **文字输入框**（自定义需求） | 填写需求后点击「开始分析」，调用 Claude AI | **按量计费** |

**只有使用右侧文字输入框时才产生 API 费用。** 按钮功能（包括转录、说话人识别、视频剪辑）全部在本机运行，不联网、不计费。
...
| 关键词提取 / 时间段截取 / 去除静音 | 本地规则引擎 | **免费** |
| **文字输入 AI 解析** | Claude Haiku API | **按量计费**，见下表 |
| HuggingFace 模型下载 | pyannote 模型约 1.5 GB | **免费**（一次性下载） |

### Claude Haiku API 费用估算（仅文字输入路径）

每次使用文字输入框点击「开始分析」调用一次 API，费用取决于视频时长（决定转录文本长度）：

| 视频时长 | 约消耗 token | 单次费用估算 |
|---------|------------|------------|
| 10 分钟 | ~3,000 tokens | 约 ¥0.03 |
| 30 分钟 | ~8,000 tokens | 约 ¥0.07 |
| 1 小时 | ~15,000 tokens | 约 ¥0.14 |
| 3 小时 | ~45,000 tokens | 约 ¥0.40 |
...
- 理解模糊描述："找情绪激动的片段"、"提取最精彩的三个观点"
- 口语时间描述："截取前五分钟"、"找第二个话题之后的内容"
- 去除口误和重复片段（AI 自动识别说错重说、语义重复的段落）

### 控制费用的建议

- **优先使用按钮**：关键词、说话人、时间段、去静音四种需求均可通过按钮免费完成，无需动用文字输入
- **同一视频多次修改**：直接在候选列表里勾选/取消片段不计费；只有再次点击「开始分析」才会重新调用 API
...
点击或拖拽视频文件到界面左侧上传区域，上传完成后自动显示视频预览。
...
界面右侧提供两种操作方式，可单独使用也可组合。

#### 方式 A — 快捷功能按钮（免费，无需 AI）

4 个功能按钮支持**多选**，点击后高亮显示，选中后展开对应的参数输入区。

**🔍 关键词提取**

- 在展开区输入关键词（逗号分隔），系统定位所有命中片段
- 可设置片段前/后留白时间（秒），默认各 3 秒
- 示例关键词：`竞品, 价格, 折扣`

**🎙 按说话人剪辑**（需配置 HuggingFace Token，见[说话人两阶段流程](#按说话人剪辑的两阶段流程)）

- 点击「开始分析」后先执行转录，界面会出现检测到的说话人列表
- 勾选目标说话人后点「确认筛选」，系统提取该说话人的全部发言，每句作为独立片段返回

**⏱ 截取时间段**

- 输入起始时间和结束时间（秒），系统截取该区间内容

**✂️ 去除静音**

- 无需额外参数，系统自动检测音量低谷，将连续有声内容作为独立片段返回（不自动拼接）

**多选组合：** 同时选中多个按钮后，规则按顺序叠加执行。例如，同时选「关键词提取 + 去除静音」，系统先找到关键词片段，再对每个片段做静音切割，返回更精确的子片段。

---

#### 方式 B — 自定义文字需求（AI 解析，少量费用）

在文字输入框中用自然语言描述剪辑需求，Claude AI 理解后自动生成对应剪辑计划。适合按钮无法满足的复杂需求：

- 理解模糊描述："找情绪激动的片段"、"提取最精彩的三个观点"
- 口语时间描述："截取前五分钟"、"找第二个话题之后的内容"
- 去除口误和重复片段（AI 自动识别说错重说、语义重复的段落）

> **费用提示**：10 分钟视频约 ¥0.03，1 小时视频约 ¥0.14，详见[使用费用说明](#使用费用说明)。

---

### 第三步：点击「开始分析」

点击「🚀 开始分析」后，界面下方出现处理进度区，步骤条实时更新：

| 步骤 | 说明 | 参考耗时 |
|------|------|---------|
| ① 音频转录 | WhisperX 将视频音频转为带时间戳的文字（可含说话人分离） | 10 分钟视频约 1–3 分钟 |
| ② 解析意图 | Claude Haiku 理解文字需求，生成剪辑计划（**仅文字输入路径**） | 约 5–10 秒 |
| ③ 执行规则 | 在转录结果中匹配候选片段 | 约 1 秒 |

**使用按钮路径时跳过第②步，直接从①到③，无 API 调用。**

---

### 按说话人剪辑的两阶段流程

选中「🎙 按说话人剪辑」按钮时，需要两次交互：
...
2. 点击「开始分析」
3. 等待转录完成（1–3 分钟）
4. 界面出现说话人选择器，列出检测到的说话人（SPEAKER_00、SPEAKER_01...）

**第二阶段：筛选并提取片段**

5. 勾选目标说话人（可多选）
6. 点击「确认筛选」
7. 系统提取该说话人的全部发言片段并显示结果

> **说话人分离需要 HuggingFace Token**，未配置时「按说话人剪辑」功能不可用，其余功能正常。

---

### 第四步：审核候选片段

处理完成后，进度区被结果区替换，步骤条全部变绿：
...
| # | 说话人 | 时间范围 | 内容预览 | 置信度 | 包含 |
|---|--------|---------|---------|------|-----|
| 1 | SPEAKER_00 | 12.3s – 28.5s | 竞品的价格策略非常激进… | 0.95 | ☑ |
| 2 | SPEAKER_01 | 45.0s – 62.1s | 跟竞品相比，续航方面… | 0.88 | ☑ |
| 3 | SPEAKER_00 | 120.5s – 135.0s | 说到价格，用户反馈最多的是… | 0.92 | ☑ |

- **取消勾选**不需要的片段，只保留最终要导出的内容
- **说话人列**：配置 HuggingFace Token 后显示 SPEAKER_xx 标签；未配置时显示"—"
- 时间范围和内容预览帮助你快速判断是否保留
...
导出完成后界面出现文件下载列表，每个片段可单独下载，文件同时保存在项目 `output/` 目录。
...
│   └── pipeline.py          ← 纯函数辅助层（按钮路径 EditPlan 构建、片段格式化）
├── agent/
